...
struct ArtworkDetailView: View {
    ...
    @StateObject private var overviewViewModel = ArtworkOverviewViewModel()
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    if !overviewViewModel.hasGenerated {
                        Text("Get AI-generated insights about this artwork")
                            .font(.system(size: 14))
                ...
                Button(action: {
                    Task {
                        await overviewViewModel.generateOverview(for: artwork, exhibition: exhibition)
                    }
                }) {
                    HStack(spacing: 6) {
                        if overviewViewModel.isLoading {
                            ...
                            Image(systemName: overviewViewModel.hasGenerated ? "arrow.clockwise" : "sparkles")
                                .font(.system(size: 14, weight: .medium))
                        }
                        
                        Text(overviewViewModel.hasGenerated ? "Regenerate" : "Generate Overview")
                            .font(.system(size: 14, weight: .medium))
                    }
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 8)
                            .fill(overviewViewModel.isLoading ? Color.gray : Color.blue)
                    )
                }
                .disabled(overviewViewModel.isLoading)
            }
            
            // Overview content
            if overviewViewModel.hasGenerated && !overviewViewModel.overview.isEmpty {
                VStack(alignment: .leading, spacing: 12) {
                    Text(overviewViewModel.overview)
                        .font(.system(size: 16))
                        .foregroundColor(.primary)
                        .multilineTextAlignment(.leading)
                        .fixedSize(horizontal: false, vertical: true)
                    ...
                    HStack {
                        ...
                        Text("Generated by AI • May contain inaccuracies")
                            .font(.system(size: 12))
                            .foregroundColor(.secondary)
            ...
            // Error state
            if let errorMessage = overviewViewModel.errorMessage {
                VStack(alignment: .leading, spacing: 8) {
                    HStack {
                        Image(systemName: "exclamationmark.triangle")
                            .foregroundColor(.orange)
                        
                        Text("Unable to generate overview")
                            .font(.system(size: 14, weight: .medium))
                            .foregroundColor(.orange)
                    }
